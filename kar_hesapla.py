...
# 2. Amazon Raporlarını Oku ve Birleştir
amazon_df_list = []
...
# 🧮 DÜNKÜ ÇALIŞAN MATEMATİK MOTORU
total_revenue = 0.0
total_amazon_fees = 0.0
total_product_cost = 0.0

# Sipariş satırları üzerinde dönüp maliyetleri ve kârları karşılaştıralım
for index, row in combined_amazon.iterrows():
    # Tutar sütununu esnek olarak çekelim
    amount_val = row.get('amount', row.get('Tutar', row.get('total', row.get('toplam', 0))))
    try:
    ...
    sku_str = str(row.get('seller-sku', row.get('Stok Kodu', row.get('sku', row.get('Sku', ''))))).strip().upper()
    
    # Sipariş ve Satışları yakalayan dün yazdığımız asıl filtre
