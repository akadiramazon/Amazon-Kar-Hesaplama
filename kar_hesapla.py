...
st.markdown("Geniş Maliyet (32 Sütunlu) Veri Bankası Entegrasyonu Aktiftir. Fiyat ve maliyetleri doğrudan geniş listeden canlı okur.")
st.markdown("---")

# 📊 DOSYA YÜKLEME ALANLARI (SOL MENÜ)
st.sidebar.header("📦 Veri Yükleme Merkezi")
maliyet_file = st.sidebar.file_uploader("1️⃣ genişmaliyet.csv Dosyasını Seçin", type=["csv"], key="maliyet")
...
if not maliyet_file or not amazon_files:
    st.info("💡 Paneli çalıştırmak için sol menüden **genişmaliyet.csv** dosyanızı ve **Amazon Finans Raporlarınızı** seçin kanka!")
    st.stop()

try:
    # 1. Geniş Maliyet Dosyasını Oku (genişmaliyet.csv)
    ...
    # Geniş maliyet dosyanızdaki orijinal sütun adları
    urun_adi_col = "ÜRÜN ADI"
    maliyet_col = "KDV li Maaliyet"
    satis_col = "GERÇEK SATIŞ FİYATI"

    # Güvenlik Kontrolü: Doğru dosya yüklendi mi?
    if urun_adi_col not in df_master.columns or maliyet_col not in df_master.columns:
        st.error(f"🚨 HATA: Yüklediğiniz dosyada '{urun_adi_col}' veya '{maliyet_col}' sütunu bulunamadı! Lütfen 3 sütunluk listeyi değil, asıl 32 sütunluk 'genişmaliyet.csv' dosyanızı yükleyin kanka.")
        st.stop()
    ...
    # Sayısal Dönüştürme Motoru (22.760,21 TL gibi noktalı/virgüllü formatları kusursuz çözer)
    ...
    # Fiyatları ve maliyetleri geniş listenizden anlık olarak sayıya çeviriyoruz
    ...
    # 🎯 GENİŞ VERİ TABANLI CANLI HİBRİT EŞLEŞTİRME MOTORU
    ...
    for name in unique_amazon_names:
        ...
        # 3. Aşama: Gelişmiş içerik araması (İlk 15 karakter güvenlik sınırı ile kelime havuzundan bulur)
        if matches.empty:
    ...
    st.markdown("---")
    st.subheader("📋 %100 Kusursuz Kârlılık Rapor Tablosu")
